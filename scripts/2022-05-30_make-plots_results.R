...
d.with.performance <- d.abridged %>%
  ...
  mutate(
    ...
    `Contribution of squared bias` = (mean(est) - true.value)^2/var(est),
    Variance = var(est),
    n.converged.datasets = n(),
    nonconverged.percent = round(100 * (1000 - n()) / 1000, 2)
  ) %>%
  # mutate(
  #   `Contribution of squared bias` = Bias^2/Variance,
  # ) %>%
  group_by(sign.X.sig,
           .add = TRUE) %>%
  filter(sign.X.sig == "Positive") %>%
  ## here we decide to use proportions of percentage
  mutate(`Sampling error` = mean(est) - true.value,
         `Sampling error contribution` = (mean(est) - true.value)^2/var(est),
         percent = round(n() / n.converged.datasets, 3),
         .after = sign.X.sig) %>%
...
plot_Model.x.Resid <- function(d_important,
  ...
  if(which.measure == "Type-1 error"){
    output.plot <- output.plot +
      scale_y_continuous(breaks = c(0, 0.05, 0.10, 0.20, 0.40, 0.60),
...
plot_quadrants <- function(d_important,
                           which.parameter = "Covariance",
                           upper.measure = "Bias",
                           lower.measure = "Sampling error",
                           line.width = 2,
                           font.scale = 8){
